...
A simple shoppingcart implementation for Laravel. (더 이상 업데이트되지않아 포크하여 사용하고 있으며, 3.0.0 부터 라라벨 10.x 를 지원합니다.)
...
Run the Composer require command from the Terminal:
    ...
    composer require artgrammer/shoppingcart
...
Add a new line to the `providers` array:
	...
	SaAkSin\Shoppingcart\ShoppingcartServiceProvider::class

And optionally add a new line to the `aliases` array:

	'Cart' => SaAkSin\Shoppingcart\Facades\Cart::class,
...
If you want to change these options, you'll have to publish the `config` file.
    ...
    php artisan vendor:publish --provider="SaAkSin\Shoppingcart\ShoppingcartServiceProvider" --tag="config"

This will give you a `cart.php` config file in which you can make the changes.

To make your life easy, the package also includes a ready to use `migration` which you can publish by running:

    php artisan vendor:publish --provider="SaAkSin\Shoppingcart\ShoppingcartServiceProvider" --tag="migrations"
